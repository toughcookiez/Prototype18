...
public class FirstPersonController : MonoBehaviour
{
    ...
    public Sprite hitFeedbackSprite;

    // Internal Variables
    private float yaw = 0.0f;
    private float pitch = 0.0f;
    private Image crosshairObject;
    private HitCrosshairFeedback hitCrosshairFeedback;
    [Header("Ammo UI")]
    ...
    void SetupAmmoDisplay()
    {
        if (ammoDisplay == null)
        {
            ammoDisplay = GetComponentInChildren<AmmoDisplay>(true);
        }

        if (ammoDisplay == null)
        {
            Debug.LogWarning("AmmoDisplay not found. Add an AmmoDisplay component to the player Canvas and assign it in FirstPersonController.", this);
            return;
        }
        ...
        if (weaponManager != null)
        {
            ammoDisplay.SetWeaponManager(weaponManager);
            return;
        }
        ...
        {
            ammoDisplay.SetWeapon(weaponHandler);
        }
        else
        {
            Debug.LogWarning("No WeaponManager or WeaponHandler found for AmmoDisplay binding.", this);
        }
    }

    void OnActiveWeaponChanged(WeaponHandler weapon)
    {
        if (weapon != null && hitCrosshairFeedback != null)
        {
            weapon.hitFeedback = hitCrosshairFeedback;
        }
    }
    ...
    void SetupHitFeedback()
    {
        ...
        // Try to find existing HitCrosshairFeedback component
        hitCrosshairFeedback = GetComponentInChildren<HitCrosshairFeedback>();

        if (hitCrosshairFeedback == null)
        {
            // Create hit feedback UI if it doesn't exist
            // Find the Canvas that contains the crosshair
            Canvas canvas = GetComponentInChildren<Canvas>();
            if (canvas != null)
            {
                // Create the HitFeedback GameObject
                GameObject hitFeedbackGO = new GameObject("HitFeedback");
                hitFeedbackGO.transform.SetParent(canvas.transform, false);

                // Setup RectTransform (same as Reticle)
                RectTransform rectTransform = hitFeedbackGO.AddComponent<RectTransform>();
                ...
                // Add Image component
                Image hitFeedbackImage = hitFeedbackGO.AddComponent<Image>();
                hitFeedbackImage.sprite = hitFeedbackSprite != null ? hitFeedbackSprite : crosshairImage;
                hitFeedbackImage.raycastTarget = false;

                // Add HitCrosshairFeedback component
                hitCrosshairFeedback = hitFeedbackGO.AddComponent<HitCrosshairFeedback>();
                hitCrosshairFeedback.hitFeedbackImage = hitFeedbackImage;
                hitCrosshairFeedback.hitCrosshairSprite = hitFeedbackImage.sprite;
            }
        }

        // Connect to WeaponHandler
        if (hitCrosshairFeedback != null)
        {
            WeaponHandler[] weaponHandlers = GetComponentsInChildren<WeaponHandler>(true);
            for (int i = 0; i < weaponHandlers.Length; i++)
            {
                if (weaponHandlers[i] != null)
                {
                    weaponHandlers[i].hitFeedback = hitCrosshairFeedback;
                }
            }
